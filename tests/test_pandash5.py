...
def nodes():
    return pd.DataFrame({"x": [1, 2, 3, 4] * 3, "y": [1] * 4 + [2] * 4 + [3] * 4})


@pytest.fixture(scope="module")
def edges():
    return pd.DataFrame(
        {"from": [0, 4, 5, 6, 2, 2, 6, 10, 9, 7], "to": [4, 5, 6, 2, 1, 3, 10, 9, 8, 11]}
    )
...
def edge_weights(edges, impedance_names):
    return pd.DataFrame(
        {impedance_names[0]: [1] * len(edges), impedance_names[1]: list(range(1, len(edges) + 1))}
    )
...
def test_remove_nodes(network, rm_nodes):
    ...
    exp_nodes = pd.DataFrame(
        {"x": [2, 3, 4, 1, 2, 1, 2, 3, 4], "y": [1, 1, 1, 2, 2, 3, 3, 3, 3]},
...
def test_network_to_pandas_hdf5(tmpfile, network, nodes, edges_df, impedance_names, two_way):
    ...
    with pd.HDFStore(tmpfile) as store:
        ...
        assert_series_equal(store["impedance_names"], pd.Series(impedance_names))


def test_network_to_pandas_hdf5_removal(tmpfile, network, impedance_names, two_way, rm_nodes):
    nodes, edges = ph5.remove_nodes(network, rm_nodes)
